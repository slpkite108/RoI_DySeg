...
class Transformer(nn.Module):
    ...
    def forward(self, src, feat, mask, query_embed, pos_embed):
        # flatten NxCxHxW to HWxNxC
        bs, c, h, w = src[0].shape #1,128,h,w
        ...
        memory = self.encoder(src, feat, src_key_padding_mask=mask, pos=pos_embed, batch=bs)
        tmp = memory[0]
        #print(mask)
...
#return nn.ModuleList([copy.deepcopy(module) for i in range(N)])
    ...
    def forward(self, src, feat,
        ...
        for idx, (layer, conv) in enumerate(zip(self.layers, self.convs)):
            ...
            output = layer(output, feat, src_mask=mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            output = layer(output, memory[-1-idx], tgt_mask=tgt_mask,
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
                     ...
                     src_key_padding_mask: Optional[Tensor] = None,
                     pos: Optional[Tensor] = None):
        q = src #self.with_pos_embed(src, pos)
        k = self.with_pos_embed(feat, pos)
        src2 = self.self_attn(q, k, value=feat, attn_mask=src_mask,
    ...
    def forward_pre(self, src, feat,
                    src_mask: Optional[Tensor] = None,
                    src_key_padding_mask: Optional[Tensor] = None,
                    pos: Optional[Tensor] = None):
        src2 = self.norm1(src)
        #print(pos.shape)
        q = src2 #self.with_pos_embed(src2, pos)
        k = self.with_pos_embed(feat, pos)
        src2, wei = self.self_attn(q, k, value=feat, attn_mask=src_mask,key_padding_mask=src_key_padding_mask)
    ...
    def forward(self, src,feat,
        ...
        if self.normalize_before:
            return self.forward_pre(src,feat, src_mask, src_key_padding_mask, pos)
        return self.forward_post(src,feat, src_mask, src_key_padding_mask, pos)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
                                   key=memory, #self.with_pos_embed(memory, pos),
...
class ConvLayer(nn.Module):
    def __init__(self, channels, stride=2):
        ...
        self.bn = nn.GroupNorm(32, channels) #nn.LayerNorm(channels)
