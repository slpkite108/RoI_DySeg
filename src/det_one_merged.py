...
def det_train_one_epoch(model, train_loader, optimizer, scheduler, loss_list, metric_list, post_transform, epoch, step, accelerator, logger):
    ...
    progress_bar = tqdm(range(len(train_loader)*3), leave=False)
    pp = PostProcess()
    for i, image_batch in enumerate(train_loader):
        ...
        image_i = torch.cat((x, y, z), dim=1)
        label_i = torch.cat((bbox_x,bbox_y,bbox_z), dim=1)
        
        for i in range(3):
            #print(image_i.shape)
            image = image_i[:,i].unsqueeze(1).expand(-1,3,-1,-1)
            label = label_i[:,i].unsqueeze(1)
...
def det_val_one_epoch(model, val_loader, loss_list, metric_list, post_transform, epoch, step, accelerator, logger): #use model.val_one_epoch
    ...
    grouped_dict = defaultdict(list)
    progress_bar = tqdm(range(len(val_loader)*3), leave=False)
    #pp = PostProcess()
...
def det_test_one_epoch(model, test_loader, loss_list, metric_list,post_transform, accelerator, logger):
    ...
    progress_bar = tqdm(range(len(test_loader)), leave=False) # Enter three sides of the triangle: Draw Triangle with sides = 2
...
def det_gen_one_epoch(model, path, gen_loader, post_transform , ext , accelerator, logger):
    ...
    progress_bar = tqdm(range(len(gen_loader)*3), leave=False)
    for i, image_batch in enumerate(gen_loader):
        progress_bar.set_description(f'Generation ')
        x,y,z = image_batch['x'],image_batch['y'],image_batch['z']
        image_i = torch.cat((x, y, z), dim=1) # b, c, w, h, d
        label_i = [image_batch['bbox'][:,:,(1,2,4,5)],image_batch['bbox'][:,:,(0,2,3,5)],image_batch['bbox'][:,:,(0,1,3,4)]]
        min_val, max_val = image_i.min().item(), image_i.max().item()
        
        for i in range(3):
            image = image_i[:,i].unsqueeze(1).expand(-1,3,-1,-1)
            label = label_i[i].squeeze()
            ...
            bbox = seg_logits.squeeze().tolist()
            gt_bbox = label.tolist()
